...
2. Remove direct `dio`, `dart_either`, `either_dart`, `flutter_easyloading`, `web_socket_channel`, and `connectivity_plus` dependencies unless another package truly needs them.
...
4. Create or update API setup **only for features the app wants**:
   - `OneRequest.configure(baseUrl: ...)` as a default only; use `OneRequest(baseUrl: …)` per extra backend
   ...
   - `OneRequest.socket(...)` only if the app wants WebSockets (one instance per URL; host may differ from HTTP)
   - `OneRequest.setConnectivity(...)` only if the app wants connectivity_plus notices
